    ...
    pub(crate) fn lower_const_def(
        ...
        let lowered = match &constant.kind {
            ...
            ConstantKind::GetElementPointer {
                ...
                // pointee type for a typed multi-index access), and the
                // frontend VALIDATED the indices against it at conversion
                // (const_instructions.rs), refusing unrecoverable shapes.
                // Same contract as the runtime GEP lowering in `value.rs`.
                let result = unsafe {
                    if *in_bounds {
...
mod tests {
    ...
    fn const_global_load_i64() {
        ...
        rvsdg
            .define_fn(func_id, |rb| {
                let ptr = rb.global_ref(global_id, ptr_ty);
                let loaded = rb.load(ptr, I64, None, false);
                Ok(vec![loaded])
            })
            .unwrap();
    ...
    fn const_global_load_f32() {
        ...
        rvsdg
            .define_fn(func_id, |rb| {
                let ptr = rb.global_ref(global_id, ptr_ty);
                let loaded = rb.load(ptr, F32, None, false);
                Ok(vec![loaded])
            })
            .unwrap();
    ...
    fn const_aggregate_array_i32() {
        ...
        rvsdg
            .define_fn(func_id, |rb| {
                let ptr = rb.global_ref(global_id, ptr_ty);
                let loaded = rb.load(ptr, arr_ty, None, false);
                let elem = rb.extract_field(loaded, &[1], I32);
                Ok(vec![elem])
            })
            .unwrap();
        assert_eq!(jit_run_i32(&rvsdg, "test"), 20);
    ...
    fn const_aggregate_array_sum() {
        ...
        rvsdg
            .define_fn(func_id, |rb| {
                let ptr = rb.global_ref(global_id, ptr_ty);
                let loaded = rb.load(ptr, arr_ty, None, false);
                let e0 = rb.extract_field(loaded, &[0], I32);
                let e1 = rb.extract_field(loaded, &[1], I32);
                let e2 = rb.extract_field(loaded, &[2], I32);
                let e3 = rb.extract_field(loaded, &[3], I32);
    ...
    fn const_aggregate_array_f32() {
        ...
        rvsdg
            .define_fn(func_id, |rb| {
                let ptr = rb.global_ref(global_id, ptr_ty);
                let loaded = rb.load(ptr, arr_ty, None, false);
                let a = rb.extract_field(loaded, &[0], F32);
                let b = rb.extract_field(loaded, &[1], F32);
                let sum = rb.binary(BinaryOp::FloatAdd, ArithFlags::default(), a, b, F32);
                Ok(vec![sum])
            })
            .unwrap();
