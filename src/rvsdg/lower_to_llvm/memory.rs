...
impl RVSDGMod {
    ...
    pub(crate) fn lower_load<'a, 'ctx>(
        ...
        let inst = load
            ...
            .ok_or_else(|| eyre!("build_load did not produce an instruction"))?;
        if let Some(a) = align {
    ...
    pub(crate) fn lower_compare_and_swap<'a, 'ctx>(
        ...
        if volatile {
            result
                .as_instruction()
                .ok_or_else(|| eyre!("build_cmpxchg did not produce an instruction"))?
                .set_volatile(true)
                .map_err(|e| eyre!("could not mark compare-and-swap volatile: {e}"))?;
        }
    ...
    pub(crate) fn lower_atomic_load<'a, 'ctx>(
        ...
        if let Some(a) = align {
            inst.set_alignment(a)
                .map_err(|e| eyre!("invalid atomic load alignment {a}: {e}"))?;
        }
        if volatile {
            inst.set_volatile(true)
                .map_err(|e| eyre!("could not mark atomic load volatile: {e}"))?;
        }
    ...
    pub(crate) fn lower_atomic_store<'a, 'ctx>(
        ...
        if let Some(a) = align {
            store
                .set_alignment(a)
                .map_err(|e| eyre!("invalid atomic store alignment {a}: {e}"))?;
        }
        if volatile {
            store
                .set_volatile(true)
                .map_err(|e| eyre!("could not mark atomic store volatile: {e}"))?;
        }
        store
            .set_atomic_ordering(ordering_to_llvm(ordering))
            .map_err(|e| eyre!("invalid atomic store ordering {ordering:?}: {e}"))?;
        Ok(())
    }
    ...
    pub(crate) fn lower_atomic_read_modify_write<'a, 'ctx>(
        ...
        // xchg are REFUSED here -- unlike the weak-CAS and syncscope
        // compromises this is not a conservative strengthening, it rejects
        // valid IR outright. The lift is small when needed: the C-level
        // LLVMBuildAtomicRMW takes an untyped value ref, Builder::
        // as_mut_ptr is public, and FloatValue::new/PointerValue::new wrap
        // the raw result back into inkwell (roughly ten contained unsafe
        // lines mirroring build_atomicrmw's own body).
        let BasicValueEnum::IntValue(int_operand) = operand else {
            bail!(
                "atomic read-modify-write on a non-integer operand is not \
        ...
        let old_value = llvm_builder.builder.build_atomicrmw(
            ...
            ptr.into_pointer_value(),
            int_operand,
            ordering_to_llvm(ordering),
        )?;
        if volatile {
            old_value
                .as_instruction()
                .ok_or_else(|| eyre!("build_atomicrmw did not produce an instruction"))?
                .set_volatile(true)
...
/// Map the RVSDG operation to inkwell's. Float operations are modeled in
/// the RVSDG but rejected here: inkwell's build_atomicrmw is integer-only
/// (see lower_atomic_read_modify_write for the consequence and the
/// llvm-sys escape hatch that would lift it).
fn atomic_read_modify_write_op_to_llvm(op: AtomicRMWOp) -> color_eyre::Result<AtomicRMWBinOp> {
    Ok(match op {
        AtomicRMWOp::Exchange => AtomicRMWBinOp::Xchg,
        ...
        AtomicRMWOp::SignedMax => AtomicRMWBinOp::Max,
        AtomicRMWOp::SignedMin => AtomicRMWBinOp::Min,
        AtomicRMWOp::UnsignedMax => AtomicRMWBinOp::UMax,
        AtomicRMWOp::UnsignedMin => AtomicRMWBinOp::UMin,
        AtomicRMWOp::FloatAdd
        | AtomicRMWOp::FloatSub
        | AtomicRMWOp::FloatMax
        | AtomicRMWOp::FloatMin => bail!(
            "float atomic read-modify-write ({op:?}) is not supported by \
             inkwell's builder"
        ),
    })
}
